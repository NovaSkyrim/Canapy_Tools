...
def split_train_test(corpus, *, test_set=None, redo=False, **kwargs):
    ...
    test_set: Optional, DataFrame or list of sequence IDs that will be used as the test set.
    ...
    if test_set is not None:
        if isinstance(test_set, pd.DataFrame):
            test_seqs = test_set["seqid"].unique()
...
test_set_df = pd.read_csv('path_to_your_file.csv')

test_set_seqs = test_set_df['seqid'].unique()

corpus = split_train_test(corpus, test_set=test_set_seqs)
